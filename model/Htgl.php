...
class Manage{
    ...
    function add_sp($commodity){
        $sql = "insert into commodity (goodname,img,magnifying,price,particulars,Serialnumber,T1,T2,texttrue,color,size,theme,time) values ('{$commodity->goodname}','{$commodity->img}','{$commodity->magnifying}','{$commodity->price}','{$commodity->particulars}','{$commodity->Serialnumber}','{$commodity->T1}','{$commodity->T2}','{$commodity->texttrue}','{$commodity->color}','{$commodity->size}','{$commodity->theme}','{$commodity->timer}')";
        $res = $this->db->query($sql);
        if ($res){
            echo '{"code":"1"}';
        }else{
    ...
    function update_sp($com){
        //修改商品信息
            ...
            time = '{$com->timer}' where id='{$com->id}'";
            $res = $this->db->query($sql);
            if ($res){
                echo '{"code":"1"}';
            }else{
